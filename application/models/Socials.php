...
class socials extends CI_Model {
        ...
        if (isset($_SESSION['user']) and (strlen($_SESSION['user']) > 0)) {
            ...
            $sx = '
                ...
                <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                    <a class="dropdown-item" href="' . base_url($this -> url . 'social/perfil') . '">' . msg('user_perfil') . '</a>
                    <a class="dropdown-item" href="' . base_url($this -> url . 'social/logout') . '">' . msg('user_logout') . '</a>
                </div>                
                ';
        } else {
    ...
    function button_login_modal() {
                  ...
                  <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        <form method="post" action="' . base_url($this -> url . 'login') . '">
        ...
        $data['email_ok'] = '';
        $data['error'] = '';
        //$sx = $this -> load -> view($this->path.'login.php', $data, true);
        $sx = $this -> view_login($data);
        return ($sx);
    }
    ...
    function view_login($data) {
        $hide = 'true';
        $class = "modal fade";
        if (isset($data['show']) and ($data['show'] == 1)) {
            $hide = '';
            $class = '';
        }
        $sx = '<div class="' . $class . '" id="exampleModalLong" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="' . $hide . '">' . cr();
        $sx .= '    <div class="modal-dialog" role="document">' . cr();
        $sx .= '        <div class="modal-content">' . cr();
        $sx .= '            <div class="modal-header">' . cr();
        $sx .= '            <h5 class="modal-title" id="exampleModalLongTitle">' . msg("user_login") . '</h5>' . cr();
        $sx .= '            <button type="button" class="close" data-dismiss="modal" aria-label="Close">' . cr();
        $sx .= '            <span aria-hidden="true">&times;</span>' . cr();
        $sx .= '            </button>' . cr();
        $sx .= '        </div>' . cr();
        $sx .= '        <div class="modal-body">' . cr();
        $sx .= '            <form method="post" action="' . base_url($this -> url . 'social/login_local') . '">' . cr();
        $sx .= '                <span><?php echo msg("form_user_name"); ?></span>' . cr();
        $sx .= '                <br>' . cr();
        $sx .= '                <input type="text" name="user_login" value="' . get("user_login") . '" class="form-control">' . cr();
        $sx .= '                <br>' . cr();
        $sx .= '                <span>' . msg("form_user_password") . '</span>' . cr();
        $sx .= '                <br>' . cr();
        $sx .= '                <input type="password" name="user_password" value="" class="form-control">' . cr();
        $sx .= '                <br>' . cr();
        $sx .= '                <button type="button" class="btn btn-secondary" data-dismiss="modal">' . cr();
        $sx .= msg('cancel') . cr();
        $sx .= '                </button>' . cr();
        $sx .= '                &nbsp;|&nbsp;' . cr();
        $sx .= '                <input type="submit" name="action" value="' . msg('user_login') . '" class="btn btn-primary">' . cr();
        $sx .= '                </form>' . cr();
        $sx .= '            </div>' . cr();
        $sx .= '        </div>' . cr();
        $sx .= '    </div>' . cr();
    ...
    function login() {
        ...
        $this -> load -> view($this -> uri . 'social/login_signin', null);
        ...
        if ($form -> saved > 0) {
            if (($dd2 == $dd2) and (strlen($dd2) > 0)) {
                if (($p1 == $p2) or ($new == 1)) {
                    ...
                    $this -> db -> query($sql);
                    redirect(base_url($this -> uri . '/social/login'));
                } else {
