...
class Index extends Model
{
	...
	function gerarCertificadosForm($id = '', $ev = 2)
	{
		...
		$sx .= '<label for="event" class="form-label">Evento</label>' . chr(10);
		$sx .= '<select class="form-select border border-secondary" id="event" name="event">' . chr(10);
		$EventModel = new \App\Models\Event\Events();
		$events = $EventModel->orderBy('e_data_f', 'desc')->findAll();
		foreach ($events as $e) {
			$selected = '';
			if ($e['id_e'] == $ev) {
				$selected = ' selected ';
			}
			$sx .= '<option value="' . $e['id_e'] . '" ' . $selected . '>' . $e['e_name'] . ' (' . date('d/m/Y', strtotime($e['e_data_i'])) . ' a ' . date('d/m/Y', strtotime($e['e_data_f'])) . ')</option>' . chr(10);
		}
		$sx .= '</select>' . chr(10);
		$sx .= '</div>';
